...
# Load session data
session = fastf1.get_session(2025, 'Austin', 'Sprint Qualifying')
session.load(telemetry=False)
weather_data = session.laps.get_weather_data()

print("\n\nCache enabled and data loaded successfully!\n\n")

sq_sessions = dict(zip(['SQ1', 'SQ2', 'SQ3'], session.laps.split_qualifying_sessions()))
...
def best_lap_times(laps_session, driver_numbers):
    ...
    return best_times[['Driver', 'DriverNumber', 'LapTime']].rename(columns={'LapTime': 'BestSprintTime'})

sprint_shootout_time = pd.DataFrame()

for q_name, q_data in sq_sessions.items():
    total_times = best_lap_times(q_data, driver_numbers).sort_values('BestSprintTime')
    # Add seconds column, handle possible non-timedelta representations
    try:
        total_times['SprintShootoutTime'] = total_times['BestSprintTime'].dt.total_seconds()
    except Exception:
        total_times['SprintShootoutTime'] = total_times['BestSprintTime'].apply(
            lambda x: x.total_seconds() if hasattr(x, 'total_seconds') else pd.to_timedelta(x).total_seconds()
        )

    print(f'\n🏁 Total Sprint Shootout Times {q_name} 🏁\n')
    total_times.index = total_times.index + 1
    print(total_times)

    if q_name == 'SQ3':
        sprint_shootout_time = total_times[['Driver', 'SprintShootoutTime']].copy()
        sprint_shootout_time.rename(columns={'SprintShootoutTime': 'SprintShootoutTime (s)'}, inplace=True)

print(f'\n🏁 Sprint Shootout results 🏁\n')
print(sprint_shootout_time)
