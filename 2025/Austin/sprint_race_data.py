...
# Load session data
session = fastf1.get_session(2025, 'Austin', 'Sprint')
...
laps_sprint = laps[["Driver", "LapTime"]].copy()
laps_sprint.dropna(subset=["LapTime"], inplace=True)
for col in ["LapTime"]:
    laps_sprint[f"{col} (s)"] = laps_sprint[col].dt.total_seconds()
    
    # Sum number of laps per driver (only valid laps present in laps_sprint)
    laps_per_driver = laps_sprint['Driver'].value_counts().sort_index().rename_axis('Driver').reset_index(name='LapCount')
